...
/**
 * KingSlimeEnemy类 - 史莱姆王敌人
 *
 * 继承自Enemy基类，实现了特定类型敌人(史莱姆王)的初始化
 * 设置了史莱姆王的动画、属性和行为特性
 */
class KingSlimeEnemy : public Enemy
{
public:
    // 构造函数：初始化史莱姆王敌人的所有属性和动画
    KingSlimeEnemy()
    {
        ...
        anim_down.set_frame_data(tex_king_slime, 3, 4, idx_list_down); // 设置动画帧数据(纹理、3列4行、帧索引列表)

        // 设置向左移动的普通动画
        anim_left.set_loop(true);
        anim_left.set_interval(0.15);
        anim_left.set_frame_data(tex_king_slime, 3, 4, idx_list_left);

        // 设置向右移动的普通动画
        anim_right.set_loop(true);
        anim_right.set_interval(0.15);
        anim_right.set_frame_data(tex_king_slime, 3, 4, idx_list_right);

        // 设置向上移动的普通动画
        anim_up.set_loop(true);
        anim_up.set_interval(0.15);
        anim_up.set_frame_data(tex_king_slime, 3, 4, idx_list_up);

        // 设置向下移动的受击动画(使用草图纹理)
        anim_down_sketch.set_loop(true);
        anim_down_sketch.set_interval(0.15);
        anim_down_sketch.set_frame_data(tex_king_slime_sketch, 3, 4, idx_list_down);

        // 设置向左移动的受击动画(使用草图纹理)
        anim_left_sketch.set_loop(true);
        anim_left_sketch.set_interval(0.15);
        anim_left_sketch.set_frame_data(tex_king_slime_sketch, 3, 4, idx_list_left);

        // 设置向右移动的受击动画(使用草图纹理)
        anim_right_sketch.set_loop(true);
        anim_right_sketch.set_interval(0.15);
        anim_right_sketch.set_frame_data(tex_king_slime_sketch, 3, 4, idx_list_right);

        // 设置向上移动的受击动画(使用草图纹理)
        anim_up_sketch.set_loop(true);
        anim_up_sketch.set_interval(0.15);
        anim_up_sketch.set_frame_data(tex_king_slime_sketch, 3, 4, idx_list_up);

        // 从配置模板中初始化敌人属性
        max_hp = king_slim_template.health;                 // 最大生命值
        max_speed = king_slim_template.speed;               // 最大移动速度
        damage = king_slim_template.damage;                 // 攻击伤害
        reward_ratio = king_slim_template.reward_ratio;     // 击杀奖励比例
        recover_interval = king_slim_template.recover_interval; // 治疗技能冷却时间
        recover_range = king_slim_template.recover_range;       // 治疗技能范围
        recover_intensity = king_slim_template.recover_intensity; // 治疗技能强度
